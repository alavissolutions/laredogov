...
Two consequences of downloading at all. A report is opened once, and only for the filing deadlines the Elections on this site cover: the city's finance page carries every filing since 2015 and its document store answers a browser navigation in roughly twenty-five seconds, so opening all of them would be hours of somebody else's bandwidth for filings no comparison table shows. Reports from before that line are recorded as Filings, linked, and left unopened, and the run log says how many. And a report the extractor reads nothing out of records which version of the extractor gave up on it, so a later version that learns to read something opens every report the one before it could not, rather than writing them off for good.
