...
- **Identity** is the city's numeric document id for every Filing (`/home/showpublisheddocument/<id>` and `/home/showdocument?id=<id>` are the same store; the trailing ticks are a cache-buster and are dropped). Candidate identity is Election, Race, and the name on the ballot as printed in the table, falling back to the legal name only for a row the city has left blank on the ballot line (amended 2026-09-18, branch review finding 1: the legal-name cell is one the city fills in late and corrects, and an identity taken from it turns one person into two on the next run).
